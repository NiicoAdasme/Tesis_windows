...
class Frame(tk.Frame):
    ...
    def show_map(self, tipo=['tmin', 'tmax', 'pr', 'ih']):
        if tipo == 'ih':
            ...
            if len(path_netcdf) > 0:
                ...
                tk.messagebox.showerror(
                    'Ups! Se te ha olvidado el archivo', msg)
    ...
    def modal_calendar(self, tipo=['tmin', 'tmax', 'pr', 'ih'], action=['plot', 'csv']):
        ...
        lbl = tk.Label(
            top, text='Seleccione una fecha entre 1978-12-15 y 2019-01-01 (Formato: YYYY-MM-DD)')
    ...
    def modal_map(self, titulo, fecha, tipo=['tmin', 'tmax', 'pr', 'ih']):
        ...
        tk.messagebox.showinfo(
            'Imagen Guardada', 'La imagen se guardó correctamente en ' + str(res))
        top.mainloop()
    ...
    def generate_gif(self, tipo=['tmin', 'tmax', 'pr']):
        ...
        if len(res) > 0:
            tk.messagebox.showinfo(
                'GIF Guardado', 'El GIF se guardó correctamente en ' + res)
        else:
            tk.messagebox.showerror(
                'Ups! Error al generar GIF', 'Lo sentimos. Hubo un error al generar el GIF')
    ...
    def export_to_csv(self, fecha, tipo=['tmin', 'tmax', 'pr', 'ih']):
        ...
        if res == None:
            tk.messagebox.showinfo(
                'Exportado a CSV', 'Archivo CSV guardado correctamente en la carpera CSV')
        else:
            tk.messagebox.showerror(
                'Ups! Error al exportar', 'Lo sentimos. No se pudo exportar a CSV')

    def merge_netcdf(self, tmin, tmax, pr):
        res, output_path = combinar_netcdf(
            netcdf_tmin=tmin, netcdf_tmax=tmax, netcdf_pr=pr)

        print(res, output_path)

        if res == None:
            tk.messagebox.showinfo(
                'Merge exitoso', 'Se combinaron los tres archivos NetCDF de forma exitosa en la carpeta NetCDF')
            return self.modal_ih(ruta_netcdf_merged=output_path)
        else:
            tk.messagebox.showerror(
                'Ups! Error al combinar', 'Lo sentimos. No se pudo combinar los archivos NetCDF')
            return False
    ...
    def validarCampos(self):
        ...
        ruta_completa = os.path.join(
            ruta_actual, 'netcdf', 'archivo_combinado.nc')
    ...
    def ih_to_netcdf(self, ruta, fecha, zona):
        ...
        res = export_ih_to_netcdf(
            ruta_nc_combinado=ruta, fecha=fecha, zona=zona)

        if len(res) > 0:
            tk.messagebox.showinfo(
                'NetCDF Creado', 'Se creó el archivo NetCDF de forma exitosa en la carpeta NetCDF')
        else:
            tk.messagebox.showerror(
                'Ups! Error al crear NetCDF', 'Lo sentimos. No se pudo crear el archivo NetCDF')
    ...
    def modal_ih(self, ruta_netcdf_merged):
        ...
        # Fecha
        lbl = tk.Label(
            top, text='Seleccione una fecha (1978-12-15 y 2019-01-01)')
        lbl.grid(row=0, column=0, padx=10, pady=10)
        cal = DateEntry(top, selectmode='day', date_pattern='YYYY-mm-dd')
        cal.grid(row=0, column=1, padx=10, pady=10)
        ...
        self.button_date = tk.Button(top, text='Exportar a NetCDF', command=lambda: self.validar_fecha(
            fecha=cal.get_date(), zona=variable.get(), ruta=ruta_netcdf_merged))
    ...
    def show_ih(self, ruta_netcdf_fecha_zona):
        ...
        if res != False:
            ...
            # Redirigir a un modal con la img
            self.modal_ih_map(res)
            tk.messagebox.showinfo(
                'Mapa IH', 'Se creó el mapa IH de forma exitosa en la carpeta img')
        else:
            # error
            tk.messagebox.showerror(
                'Ups! Error al crear mapa IH', 'Lo sentimos. No se pudo crear el mapa IH')

    def modal_ih_map(self, imagePath):
        # Load the image
        # image= Image.open(imagePath)

        # Resize the image in the given (width, height)
        # imag=image.resize((1000, 800))

        # Conver the image in TkImage
        # img=ImageTk.PhotoImage(imag)

        img = ImageTk.PhotoImage(Image.open(imagePath).resize((1920, 960)))
        # img = tk.PhotoImage(file=imagePath) 

        top = tk.Toplevel()
        top.title('Mapa Índice de Riesgo Hídrico por Zona')
        top.attributes('-fullscreen', False)

        scrollbar = tk.Scrollbar(top)
        scrollbar2 = tk.Scrollbar(top, orient='horizontal')
